...
const Key = ({type, name, onClick}: IProps) => {
  ...
  return (
    <View className={styles.item}>
      {type == 'date' && (
        <View className={styles.name}>
          <Picker mode="date" onChange={handleDateChange} value={date}>
            <View>{`${dayjs(context.data.date).month() + 1 >= 10 ? dayjs(context.data.date).month() + 1 : '0' + (dayjs(context.data.date).month() + 1)}-${dayjs(context.data.date).date() > 10 ? dayjs(context.data.date).date() : '0' + dayjs(context.data.date).date()}`}</View>
          </Picker>
        </View>
      )}
      ...
      {type == 'number' && (
        <View className={styles.name} onClick={onClick}>{name}</View>
      )}
